...
%macro canc_flags ;
				 ...
				 THROMBO OSTEO MYELO POLY_VERA    CHRONIC_MYELO 
...
cancer_chk = ACUTE_LEUKEMIA||ANAL||BLADDER||BREAST||CHRONIC_LEUKEMIA||CNS||ENDOCRINE||FEMALEGU||GASTRO_ESOPHAGEAL||
			 ...
			 MYELOID_LEUKEMIA||OTHER_SKIN||OTHER_LYMPH||OTHER_LLEUK||OTH_MONOLEUK||OTH_MYELEUK||
			 OTH_SPELEUK||SEC_LYMPH||SEC_MN_NOS||SEC_MN_RESPDIG||SEC_MN_NEUROEND||
			 ACUTE_PAN||JM_LEUK||INSITU_MELANOMA||THROMBO||OSTEO||MYELO||POLY_VERA||CHRONIC_MYELO
...
IF &VER. = 9 THEN DO ;
	...
	IF PUT(DX5,$Ess_Thromb_59_.) = "Y" THEN DO ; THROMBO = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX5,$Osteo_myelo_59_.) = "Y" THEN DO ; OSTEO = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX5,$Myelo_Fibro_59_.) = "Y" THEN DO ; MYELO = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX5,$Poly_Vera_59_.) = "Y" THEN DO ; POLY_VERA = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX4,$Poly_Vera_49_.) = "Y" THEN DO ; POLY_VERA = 1 ; RECON_ELIG = "N" ; END ;
...
IF &VER. = 0 THEN DO ;
	...
	IF PUT(DX4,$JMLeuk_410_.) = "Y" THEN do ; JM_LEUK = 1 ;	 RECON_ELIG = "N" ; END ;	
	...
	IF PUT(DX4,$Ess_Thrombo_410_.) = "Y" THEN DO ; THROMBO = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX4,$Osteo_Myelo_410_.) = "Y" THEN DO ; OSTEO = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX5,$Myelo_Fibro_510_.) = "Y" THEN DO ; MYELO = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX3,$Poly_Vera_310_.) = "Y" THEN DO ; POLY_VERA = 1 ; RECON_ELIG = "N" ; END ;
	IF PUT(DX4,$Chro_Myelo_410_.) = "Y" THEN DO ; CHRONIC_MYELO = 1 ; RECON_ELIG = "N" ; END ;
...
format cancer $100. ;
	...
	IF MN_OTH = 1 then cancer = "Malignant neoplasm of other specified ill-defined sites";	  
	...
	IF SEC_LYMPH = 1 then cancer = "Secondary malignant neoplasm of lymph nodes" ;
	IF SEC_MN_NOS = 1 then cancer = "Secondary malignant neoplasm NOS" ;
	IF SEC_MN_RESPDIG = 1 then cancer = "Secondary malignant neoplasm of resp and digestive organs" ; 
	IF SEC_MN_NEUROEND = 1 then cancer = "Secondary neuroendocrine tumors" ;

	IF THROMBO = 1 THEN CANCER = "Essential (hemorrhagic) thrombocythemia" ;
	IF OSTEO = 1 THEN CANCER = "Osteomyelofibrosis"  ;
	IF MYELO = 1 THEN CANCER = "Myelofibrosis" ;
	IF POLY_VERA = 1 THEN CANCER = "Polycythemia vera" ;
	IF CHRONIC_MYELO = 1 THEN CANCER = "Chronic myeloproliferative disease";
run ;
%MEND assign_cancer ;
